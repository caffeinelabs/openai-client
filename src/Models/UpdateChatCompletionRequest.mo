...
module {
    ...
    public module JSON {
        public func toCandidValue(value : UpdateChatCompletionRequest) : Candid.Candid {
            ...
            List.add(buf, ("metadata", #Record(Array.map<(Text, Text), (Text, Candid.Candid)>(Array.fromIter(entries(value.metadata)), func((k, v) : (Text, Text)) : (Text, Candid.Candid) = (k, #Text(v))))));
        ...
        public func fromCandidValue(candid : Candid.Candid) : ?UpdateChatCompletionRequest =
            switch (candid) {
                case (#Record(fields)) {
                    let ?metadata_field = Array.find<(Text, Candid.Candid)>(fields, func((k, _) : (Text, Candid.Candid)) : Bool = k == "metadata") else return null;
                    let ?metadata = ((switch (metadata_field.1) {
                        case (#Record(pairs__)) {
