...
module {
    /// The required-fields slice of ChatCompletionResponseMessage — what `init` consumes.
    ...
    // it's an internal scaffold so we can express ChatCompletionResponseMessage as an
    ...
    public type ChatCompletionResponseMessage = Required and Optional;

    public module JSON {
        // `init` constructs a ChatCompletionResponseMessage from just its required fields,
        // defaulting all optional fields to `null`. Pair with record-update
        // syntax to layer in selected optionals:
        //   let req = { ChatCompletionResponseMessage.init { …required fields… } with someOpt = ?… };
        // Implementation uses Candid round-trip — Candid record subtyping fills
        // absent optional fields with null. Costs a few cycles per call (init is
        // not on a hot path) but keeps generated code compact regardless of how
        // many optional fields the model has.
        public func init(required : Required) : ChatCompletionResponseMessage {
            let ?res = from_candid(to_candid(required)) : ?ChatCompletionResponseMessage else Runtime.unreachable();
            res
        };

        public func toCandidValue(value : ChatCompletionResponseMessage) : Candid.Candid {
