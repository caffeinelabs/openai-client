...
module {
    /// The required-fields slice of ChatCompletionRequestMessageContentPartImageImageUrl — what `init` consumes.
    ...
    // it's an internal scaffold so we can express ChatCompletionRequestMessageContentPartImageImageUrl as an
    // `and`-intersection and keep `init` from listing every optional explicitly.
    type Optional = {
        detail : ?ChatCompletionRequestMessageContentPartImageImageUrlDetail;
    };

    public type ChatCompletionRequestMessageContentPartImageImageUrl = Required and Optional;

    public module JSON {
        // `init` constructs a ChatCompletionRequestMessageContentPartImageImageUrl from just its required fields,
        // defaulting all optional fields to `null`. Pair with record-update
        // syntax to layer in selected optionals:
        //   let req = { ChatCompletionRequestMessageContentPartImageImageUrl.init { …required fields… } with someOpt = ?… };
        // Implementation uses Candid round-trip — Candid record subtyping fills
        // absent optional fields with null. Costs a few cycles per call (init is
        // not on a hot path) but keeps generated code compact regardless of how
        // many optional fields the model has.
        public func init(required : Required) : ChatCompletionRequestMessageContentPartImageImageUrl {
            let ?res = from_candid(to_candid(required)) : ?ChatCompletionRequestMessageContentPartImageImageUrl else Runtime.unreachable();
            res
        };
