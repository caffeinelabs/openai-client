...
module {
    ...
    public module JSON {
        ...
        public func validate(_value : CreateCompletionRequestModel) : ?Text =
            ?"openapi-generator(motoko): CreateCompletionRequestModel has no synthesisable JSON form (likely a oneOf<string, …> the codegen could not tag). Override the type or `CreateCompletionRequestModel.JSON.toJSON` in your client to fix the wire format.";
    }
}
