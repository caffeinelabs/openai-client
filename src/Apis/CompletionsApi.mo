    ...
    public func createCompletion(config : Config, createCompletionRequest : CreateCompletionRequest) : async* CreateCompletionResponse {
        ...
        if (response.status >= 200 and response.status < 300) {
            ...
            (switch (CreateCompletionResponse.fromCandidValue(_)) {
                case (?value) value;
                case null throw Error.reject("HTTP " # Int.toText(response.status) # " body[" # Nat.toText(response.body.size()) # "B]=" # (switch (Text.decodeUtf8(response.body)) { case (?t) (if (t.size() > 100) Text.fromIter(Iter.take<Char>(t.chars(), 100)) # "..." else t); case null "(undecodable)" }) # ": Failed to convert response to CreateCompletionResponse");
            })
        } else {
