...
module {
    ...
    public func createEmbedding(config : Config, createEmbeddingRequest : CreateEmbeddingRequest) : async* CreateEmbeddingResponse {
        let {baseUrl; cycles} = config;
        let baseUrl__ = baseUrl # "/embeddings";
        ...
        let request : http_request_args = { config with
            ...
            headers;
            body = do ? {
                let candidValue : Candid.Candid = CreateEmbeddingRequest.toCandidValue(createEmbeddingRequest);
        ...
        if (response.status >= 200 and response.status < 300) {
            ...
            (switch (CreateEmbeddingResponse.fromCandidValue(_)) {
                case (?value) value;
                case null throw Error.reject("HTTP " # Int.toText(response.status) # " body[" # Nat.toText(response.body.size()) # "B]=" # (switch (Text.decodeUtf8(response.body)) { case (?t) (if (t.size() > 100) Text.fromIter(Iter.take<Char>(t.chars(), 100)) # "..." else t); case null "(undecodable)" }) # ": Failed to convert response to CreateEmbeddingResponse");
            })
        } else {
    ...
    public module class EmbeddingsApi(config : Config) {
        /// Creates an embedding vector representing the input text.
        public func createEmbedding(createEmbeddingRequest : CreateEmbeddingRequest) : async CreateEmbeddingResponse {
            await* operations__.createEmbedding(config, createEmbeddingRequest)
        };
