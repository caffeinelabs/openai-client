    ...
    public func createEmbedding(config : Config, createEmbeddingRequest : CreateEmbeddingRequest) : async* CreateEmbeddingResponse {
        ...
        if (response.status >= 200 and response.status < 300) {
            ...
            (switch (CreateEmbeddingResponse.fromCandidValue(_)) {
                case (?value) value;
                case null throw Error.reject("HTTP " # Int.toText(response.status) # ": Failed to convert response to CreateEmbeddingResponse" # " — server returned: " # (switch (Text.decodeUtf8(response.body)) { case (?t) t; case null "(undecodable bytes)" }));
            })
        } else {
